...
library("gridExtra")
SuppFigure1 <- grid.arrange(Corn_RNAnoDNA_Fig,Bean_RNAnoDNA_Fig,Corn_RNA1DNA0_Fig,Bean_RNA1DNA0_Fig, ncol = 2,nrow=2, widths=c(2,1.7),heights = c(2, 2.7))
SuppFigure1
ggsave("SuppFig1.tiff",SuppFigure1,width=6,height=7, units="in")

#Before attempting to make SuppFig2, be sure to add 'geom_point' in the ggplots above.
library("gridExtra")
SuppFigure2 <- grid.arrange(Corn_RNAnoDNA_Fig,Bean_RNAnoDNA_Fig,Corn_RNA1DNA0_Fig,Bean_RNA1DNA0_Fig, ncol = 2,nrow=2, widths=c(2,1.7),heights = c(2, 2.7))
SuppFigure2
ggsave("SuppFig2.tiff",SuppFigure2,width=6,height=7, units="in")
...
library("grid")
tiff("Figure2.tiff",width=7, height=6,units="in",res=300)
grid.newpage()
...
pal19<-c("#89C5DA","#DA5724","#ffa500","#74D944",
         "#FF0000","#508578","#CE50CA","#0000FF",
         "#652926","#D7C1B1","#AD6F3B","#689030",
         "#CD9BCD","#0000FF","#CBD588","#D14285",
         "#5F7FC7","#673770","#8A7C64")

shape19<-c(15,15,15,15,16,16,16,16,23,23,23,23,24,24,24,24,25,25,25)

#plot it with ggplot
h1 <-ggplot(Corn_OTUs_16sRatio, aes(mean, value))+#, colour=Phylum,shape=Phylum))+
  geom_point(aes(colour=Phylum,shape=Phylum,fill=Phylum),size=1)+
  ...
  ggtitle("(A)")+
  scale_shape_manual(values=shape19)+
  scale_fill_manual(values=pal19)+
  scale_colour_manual(values=pal19)
h1
...
pal18<-c("#89C5DA","#DA5724","#ffa500","#74D944",
         "#FF0000","#508578","#CE50CA","#0000FF",
         "#652926","#D7C1B1","#AD6F3B","#689030",
         "#CD9BCD","#0000FF","#CBD588","#D14285",
         "#673770","#8A7C64")
shape18<-c(15,15,15,15,16,16,16,16,23,23,23,23,24,24,24,24,25,25)
#plot it with ggplot

h2 <-ggplot(Bean_OTUs_16sRatio, aes(mean, value))+
  geom_point(aes(colour=Phylum,shape=Phylum,fill=Phylum),size=1)+
  xlab("Average Copy Number at Phylum level")+
  coord_cartesian(ylim=c(-10,10))+
  theme_bw()+
  theme(legend.position="bottom",legend.title=element_blank(),axis.title.y=element_blank(),axis.text.y=element_blank())+
  theme(legend.text = element_text(size=6))+
  theme(text = element_text(size=7))+
  guides(colour = guide_legend(nrow = 5,byrow=TRUE))+
  scale_colour_manual(values=pal18)+
  scale_shape_manual(values=shape18)+
...
library(ggpubr)
Figure3<- ggarrange(h1, h2, ncol=2, nrow=1, widths=c(1.12,1),heights=c(1,1), common.legend = TRUE, legend="bottom")
ggsave("Figure3.tiff",Figure3,width=6,height=4, units="in")
...
Figure4
ggsave("Figure4.tiff",Figure4,width=6,height=5, units="in")
...
library("gridExtra")
SuppFig3 <- grid.arrange(Corn_RNAnoDNA_N_Fig,Bean_RNAnoDNA_N_Fig,ncol = 2, widths=c(2,1.7),heights = c(2))
SuppFig3
ggsave("SuppFig3.tiff",SuppFig3,width=6,height=3.5, units="in")
...
SuppFig4
ggsave("SuppFig4.tiff",SuppFig4,width=6,height=5, units="in")
